		...
		*/
		...
		pipeline
			...
			.forEach((feature, index) => {
				...
				if (index && n && (index % 1e4 === 0)) {
					let time = Date.now();
					time = (time - time0) * (n - index) / index;
					time = time / 1000;
					time = Math.floor(time / 60) + ':' + (100 + Math.floor(time % 60)).toString().slice(1);
					process.stderr.write(`\r   ${(100 * index / n).toFixed(2)}% - ${time}`);
				}
