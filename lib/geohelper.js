...
function convertGzippedGeoJSONSeq2Anything(fullnameIn, fullnameOut, layerName, layerType) {
	...
	if (layerName) args.push('-nln', layerName);
...
function ogrGetLayername(filename) {
	...
	let result = child_process.spawnSync('ogrinfo', [filename]);
...
async function generateUnionVRT(filenamesIn, filenameOut) {
	...
	result.push(`   <OGRVRTUnionLayer name="${ogrGuessLayername(filenameOut)}">`);
...
async function unionAndClipFeatures(filenameIn, filenameClip, filenameOut, bbox) {
	...
	let layerName = ogrGuessLayername(filenameIn);
	...
	if (bbox) argsIn = argsIn.concat(['-spat', ...box]);
	argsIn = argsIn.concat([
		'-dialect', 'SQLite',
		'-sql', `SELECT ST_Union(geometry) AS geometry FROM "${layerName}"`,
		'-clipdst', filenameClip,
		'-f', 'GeoJSONSeq',
		'-nlt', 'MultiPolygon',
		'/vsistdout/',
	...
	let cpOgrIn = getSpawn('ogr2ogr', argsIn);
...
async function unionAndClipFeaturesDC(filenameIn, filenameClip, filenameOut) {
	...
	let clipFeatures = JSON.parse(fs.readFileSync(filenameClip));
	let bboxAll = turf.bbox(clipFeatures);
	let [x0, y0, x1, y1] = bboxAll.map(v => Math.floor(v / scale));

	let tempFilenameOut = prefixFilename('ok', filenameOut) + '.geojson.gz';
	let tempFilenameLeft = prefixFilename('left', filenameOut) + '.geojson.gz';

	let fileOut = GzipFileWriter(tempFilenameOut);
	let fileLeft = GzipFileWriter(tempFilenameLeft);

	for (let x = x0; x <= x1; x++) {
		for (let y = y0; y <= y1; y++) {
			console.log(x,y);
			let tempFilenameBlock = calcTemporaryFilename(filenameOut)+`-block-${x}-${y}.geojson.gz`;
			if (fs.existsSync(tempFilenameBlock)) continue;
			let bboxBlock = [x, y, x + 1, y + 1].map(v => v * scale);
			await unionAndClipFeatures(filenameIn, filenameClip, tempFilenameBlock, bboxBlock);
			...
			await new Promise(res => {
				miss.each(
					...
					(line, next) => {
						...
						if (isBboxInside(bbox, bboxBlock)) {
							fileOut.write(line+'\n');
						} else {
							fileLeft.write(line+'\n');
						}
	...
	//convertGzippedGeoJSONSeq2Anything(tempFilenameOut, filenameOut, 'layer', 'Polygon');
