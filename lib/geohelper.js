...
function processAlkis(opt) {
	...
	return new Promise(resolve => {
		...
		if (opt.filenameIn.endsWith('.geojsonl')) {
			...
			n = parseInt(n, 10);
			pipeline = miss.pipeline(
				ogrLoadGpkgAsGeojsonStream(opt.filenameIn),
				miss.split()
			)
		} else {
		...
		miss.each(pipeline, async (line, next) => {
			...
			for (let file of filesOut.values()) {
				...
				//convertGzippedGeoJSONSeq2Anything(file.filenameGeoJSONSeq, file.filenameOut + '.fgb');
				//if (opt.generateGPKG) {
				convertGzippedGeoJSONSeq2Anything(file.filenameGeoJSONSeq, file.filenameOut + '.gpkg');
				//}
			}
...
function convertGzippedGeoJSONSeq2Anything(fullnameIn, fullnameOut, layerType) {
	const filenameOut = basename(fullnameOut);
	const layerName = 'layer';
	const extensionOut = filenameOut.replace(/.*?\./, '.');
	const fullnameTmp = config.getFilename.temporary() + extensionOut;

	if (fs.existsSync(fullnameOut)) fs.rmSync(fullnameOut);
	...
	args.push(fullnameTmp, ogrWrapFileDriver(fullnameIn));

	process.stdout.write(`generate ${filenameOut}: `)
	child_process.spawnSync('ogr2ogr', args, { stdio: 'inherit' })
...
async function generateUnionVRT(filenamesIn, filenameOut) {
	...
	for (let filenameIn of filenamesIn) {
		if (fs.statSync(filenameIn).size <= 20) continue; // ignore empty files
		result.push(`      <OGRVRTLayer name="${await ogrGetLayername(filenameIn)}">`)
...
async function unionAndClipFeatures(filenameIn, filenameClip, filenameOut, bbox) {
	...
	let stream = ogrLoadGpkgAsGeojsonStream(filenameIn, {union:true, bbox, filenameClip});
...
async function unionAndClipFeaturesDC(filenameIn, filenameClip, filenameOut) {
	...
	for (let x = x0; x <= x1; x++) {
		for (let y = y0; y <= y1; y++) {
			...
			let tempFilenameClip = calcTemporaryFilename(filenameOut) + `-block-${x}-${y}.geojson`;
	...
	for (let i = 0; i < blocks.length; i++) {
		let { bbox,x,y, tempFilenameBlock, tempFilenameClip } = blocks[i];
		console.log(x / scale, y / scale, (100*i/blocks.length).toFixed(1)+'%');
	...
	convertGzippedGeoJSONSeq2Anything(tempFilenameOut, filenameOut, 'Polygon');

	fs.rmSync(tempFilenameLeft1, { force:true });
	fs.rmSync(tempFilenameLeft2, { force:true });
	fs.rmSync(tempFilenameOut, { force:true });

	blocks.forEach(bbox => {
		fs.rmSync(bbox.tempFilenameBlock, { force:true })
		fs.rmSync(bbox.tempFilenameClip, { force:true })
	});
}
 ...
 * @param {object} [opt]
 * @param {boolean} [opt.dropProperties=false] drop properties and return only geometries
 * @param {array} [opt.bbox=false] use a bbox as spatial filter
 * @param {boolean} [opt.union=false] merge (Boolean UNION) all geometries to a single multipolygon
 * @param {string} [opt.clipping=false] clip results by this geometry
 * @param {string} [opt.layername=false] set layer name
 * @returns stream.Readable
 */
function ogrLoadGpkgAsGeojsonStream(filenameIn, opt) {
	opt = Object.assign({}, opt);
	
	let properties = true, bbox = false, union = false, filenameClip = false, layername=false;

	if (opt.dropProperties) properties = false;
	delete opt.dropProperties;
	
	if (opt.bbox) bbox = opt.bbox;
	delete opt.bbox;
	
	if (opt.union) union = true;
	delete opt.union;
	
	if (opt.clipping) {
		if (typeof opt.clipping === 'string') {
			filenameClip = opt.clipping;
		} else throw Error('unknown clipping value')
	}
	delete opt.clipping;
	
	if (opt.layername) layername = opt.layername;
	delete opt.layername;

	if (union && properties) throw Error('can not union, and return properties');

	if (Object.keys(opt).length > 0) throw Error('unknown options: '+Object.keys(opt).join(', '))



	let layerName = layername || ogrGetLayername(filenameIn)
