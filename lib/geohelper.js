...
function processAlkis(slug, ruleTypes, cbFeature, cbWindEntries) {
	let groupDB = new JSONDB(resolve(__dirname, config.folders.mapGroup));

	return new Promise(resolve => {
		// prepare filenames
		let filenameGeoJSONSeq = config.getFilename.mapFeature(slug+'.geojsonl');
		let groups = [];
		...
		Havel.pipeline()
			...
			.map(feature => {
				...
				for (let groupIndex of groupIndexes.values()) {
					let id = slug+'-'+groupIndex;
					let obj = groupDB.get(id);
					if (!obj) obj = [];
					obj.push(feature);
					groupDB.set(id, obj)
				}
			...
			.finished(() => {
				...
				fs.writeFileSync(config.getFilename.mapFeature(slug+'.json'), JSON.stringify(windSummary));
...
function JSONDB(folder) {
	...
	function get(key) {
		...
		let filename = resolve(folder, key+'.json');
