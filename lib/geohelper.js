...
function GeoPecker(filename) {
	...
	function check(p) {
		...
		return turf.booleanPointInPolygon(p, cell.polygon);
	}
	...
	function createCache(p) {
		...
		layer.features.forEach(f => {
			...
			turf.flatten(f).features.forEach(polygon => {
				...
				let x0 = Math.floor((bbox[0]-xc) / CELLSIZE);
				let y0 = Math.floor((bbox[1]-yc) / CELLSIZE);
				let x1 = Math.floor((bbox[2]-xc) / CELLSIZE);
				let y1 = Math.floor((bbox[3]-yc) / CELLSIZE);

				polygon = features2Coords([polygon]);
				...
				for (let x = x0; x <= x1; x++) {
					for (let y = y0; y <= y1; y++) {
						...
						if (!cell) {
							cell = {};
							cell.bbox = [x*CELLSIZE+xc, y*CELLSIZE+yc, (x+1)*CELLSIZE+xc, (y+1)*CELLSIZE+yc];
							cell.bboxPolygon = features2Coords([turf.bboxPolygon(cell.bbox)]);
							grid.set(key, cell);
						}
						let p = polygonClipping.intersection(polygon, cell.bboxPolygon);
						if (p.length === 0) continue;
						cell.polygon = cell.polygon ? polygonClipping.union(cell.polygon, p) : p;
					}
				}
			})
		})
		...
		for (let cell of grid.values()) {
			cell.polygon = cell.polygon && coords2GeoJSON(cell.polygon);
		}
	}
}
