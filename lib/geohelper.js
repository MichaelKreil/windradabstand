		...
		pipeline
			...
			.forEach(async (feature, index) => {
				...
				for (let type of types) {
					...
					if (!filesOut.has(type)) {
						let filenameOut = config.getFilename.mapFeature(type + '.geojsonl.gz');
						let file = new NDJSONGzipWrite(filenameOut);
			...
			.finished(async () => {
				...
				for (let file of filesOut.values()) {
					...
					child_process.spawnSync('ogr2ogr', ['-f', 'FlatGeoBuf', '-overwrite', '-progress', filenameTmp, 'GeoJSONSeq:/vsigzip/' + file.filename], { stdio: 'inherit' })
