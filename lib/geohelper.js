...
function convertGzippedGeoJSONSeq2Anything(fullnameIn, fullnameOut, layerName, layerType) {
	...
	if (!layerName) layerName = ogrCalcLayername(fullnameOut);
...
function ogrCalcLayername(filename) {
...
function generateUnionVRT(filenamesIn, filenameOut) {
	...
	result.push(`   <OGRVRTUnionLayer name="${ogrCalcLayername(filenameOut)}">`);
	filenamesIn.forEach(filenameIn => {
		if (fs.statSync(filenameIn).size <= 20) return; // ignore empty files
		result.push(`      <OGRVRTLayer name="${ogrCalcLayername(filenameIn)}">`)
