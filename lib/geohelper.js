...
async function convertGzippedGeoJSONSeq2Anything(fullnameIn, fullnameOut, {layerType, dropProperties}) {
	...
	args.push('--config', 'CPL_VSIL_GZIP_WRITE_PROPERTIES', 'NO')
	args.push('-progress')
	args.push('-nln', 'layer')
	args.push('-lco', 'GEOMETRY_NAME=geometry')
	if (layerType) args.push('-nlt', layerType);
	args.push(filenameTmp, ogrWrapFileDriver(fullnameIn));
	...
	fs.renameSync(filenameTmp, fullnameOut);
}
...
async function unionAndClipFeatures(filenameIn, filenameClip, filenameOut, bbox) {
	...
	let stream = ogrLoadGpkgAsGeojsonStream(filenameIn, {union:true, dropProperties:true, bbox, clipping:filenameClip});
 ...
 */
function ogrGenerateSQL(opt) {
	opt = Object.assign({}, opt);
	
	let properties = true, bbox = false, union = false, layername = 'layer';
