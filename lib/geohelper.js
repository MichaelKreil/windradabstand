		...
		pipeline
			...
			.finished(async () => {
				...
				for (let file of filesOut.values()) {
					...
					convert('.fgb', 'FlatGeoBuf');
					if (opt.generateGPKG) convert('.gpkg', 'GPKG');
						
					function convert(extension, geoType) {
						process.stdout.write(`generate ${file.name}${extension}: `)
						let filenameTmp = config.getFilename.mapFeature('tmp-' + Math.random().toString(36).slice(2) + extension);
						child_process.spawnSync('ogr2ogr', [
							...
							'-f', geoType,
							filenameTmp,
							'GeoJSONSeq:/vsigzip/' + file.filenameGeoJSONSeq
						], { stdio: 'inherit' })
						fs.renameSync(filenameTmp, file.filenameOut + extension);
					}
				}
