...
async function unionAndClipFeatures(filenameIn, filenameClip, filenameOut, bbox) {
	...
	let layerName = ogrGetLayername(filenameIn);

	let sql = `SELECT ST_Union(geometry) AS geometry FROM "${layerName}"`;
	if (bbox) {
		sql = `SELECT ST_Union(l.geometry) AS geometry FROM "${layerName}" l`
			+' JOIN "rtree_layer_geometry" r ON l.fid = r.id'
			+' WHERE r.maxx > '+bbox[0]
			+' AND r.maxy > '+bbox[1]
			+' AND r.minx < '+bbox[2]
			+' AND r.miny < '+bbox[3]
