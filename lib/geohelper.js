...
function processAlkis(slug, typeList, cbFeature, cbWindEntries) {
	return new Promise(resolve => {
		let filenameGeoJSONSeq = config.getFilename.alkisResult(slug+'.geojsonl');
		let filenameFGB = config.getFilename.alkisResult(slug+'.fgb');
		if (fs.existsSync(filenameFGB)) {
			console.log(`skip ${slug}, because ${filenameFGB} already exists`);
			return resolve();
		}

		console.log('load windfinder');

		let windFinder = WindFinder(slug, (hoehe,rad,windEntry) => {
			let result = {};
		...
		Havel.pipeline()
			.readFile(config.getFilename.alkisGeo(slug+'.geojsonl'), { showProgress: true })
			.split()
			.map(feature => {
				if (feature.length === 0) return;
				feature = JSON.parse(feature);
				
				if (!cbFeature(feature)) return;
				...
				if (cbWindEntries && !cbWindEntries(windEntries)) return;

				windEntries.forEach(([w,d]) => {
					let index = w.properties._index;
					let m = (windSummary[index] ??= { index, minDistance:{} }).minDistance;
					if ((m[type] === undefined) || (m[type] > d)) m[type] = d;
				})
			...
			.finished(() => {
				...
				fs.writeFileSync(config.getFilename.alkisResult(slug+'.json'), JSON.stringify(windSummary));

				console.log('convert to FlatGeoBuffer')
				let filenameTmp = config.getFilename.alkisResult(slug+'.tmp.fgb');
				if (fs.existsSync(filenameTmp)) fs.unlinkSync(filenameTmp);
				...
				fs.renameSync(filenameTmp, filenameFGB);
