...
function WindFinder() {
	let filenameCache = config.getFilename.helper('windFinder.json');
	let geoFinder = GeoFinder();

	if (!fs.existsSync(filenameCache)) {
		let windData = config.getFilename.wind('wind.json');
		windData = JSON.parse(fs.readFileSync(windData));
		windData = windData.map(w => turf.circle([w.Laengengrad, w.Breitengrad], w.maxRadius/1000, { properties:w }));
	...
	return feature => {
		let result = geoFinder.lookupBbox(turf.bbox(feature));
