...
async function fetchCached(filename, url, headers) {
	...
	for (let i = 1; i <= 3; i--) {
		try {
			...
			process.stdout.write('\u001b[38;5;46m.\u001b[0m');
			//await wait(200);
			break;
		} catch (code) {
			//console.log('error', code, url);
			if (code === 500) {
				process.stdout.write('\u001b[38;5;214m-\u001b[0m');
				buffer = Buffer.allocUnsafe(0);
				break;
			}

			if (code === -1) {
				process.stdout.write('\u001b[38;5;208mT\u001b[0m');
				//console.log('ETIMEDOUT, retrying', url)
				continue;
			}

			if (code !== 500) {
				process.stdout.write('\u001b[38;5;196mE\u001b[0m');
				console.log('url', url);
...
function Progress(n) {
	...
	return i => {
		let now = Date.now();
		if (now-lastTime < 300) return // only once every seconds
		lastTime = now;

		times.push([i,now]);

		let history = now - 10*1000; // based on the last 10 seconds
		while (times[0] < history) times.shift();
